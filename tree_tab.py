...
class TreeTab(SpriteNode):
    def __init__(self, transform,
                 parent=None, group=None, enabled=True, visible=True, ref=None):
        # TODO: fix dirty and rect
        self.dirty = 1
        transform = Transform(*transform)
        image = pygame.Surface((transform.width, transform.height))
        super(TreeTab, self).__init__(transform, image, parent, group, enabled, visible)
        self.ref = ref

    def draw(self, scene):
        self.dirty = 1
        if self.dirty and self.ref and self.visible:
    ...
    def recursive_draw(self, target, level=0, flat_index=0):
        ...
        for child in target:
            ...
            engine.text.draw(self.image, state, (level*18 + 9, flat_index*18), color=(255, 255, 255))
